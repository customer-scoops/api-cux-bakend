...
class Suite
{
    ...
    {   
        try{
            ...
            if($codCustomer == 'TRA001')
                $resp = DB::table($this->_dbSelected.'.'.'survey')->where('codCustomer', $codCustomer)->where('activeSurvey', 1)->where('codsurvey','TRA_VIA')->get(); //->orWhere('codsurvey','TRA_COND')
    ...
    public function getInformationDriver($searchDriver){
        $datas = [
            ...
            "jetvue_csat1"  => "Check in",
            "jetvue_csat2"  => "Registro de equipaje",
            "jetvue_csat3"  => "Embarque del vuelo",
            "jetvue_csat4"  => "Abordaje del vuelo",
            "jetvue_csat5"  => "Experiencia durante el vuelo",
            "jetvue_csat6"  => "Momento de llegada",
            "jetvue_csat7"  => "Proceso posterior al viaje",
