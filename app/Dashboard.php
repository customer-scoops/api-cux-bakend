...
class Dashboard extends Generic
{
    ...
    private function textsfilters($cod)
    {
        $arr =  [
            ...
            //JetSmart Entrega de equipaje en aeropuerto
            'hasbag1'       => 'Si',
            'hasbag0'       => 'No',
            //JetSmart Check in aeropuerto
            'hasach1'       => 'Si',
            'hasach0'       => 'No',
            //JetSmart Embarque prioritario
            'haspbd1'       => 'Si',
            'haspbd0'       => 'No',
        ];
    ...
    public function filters($request, $jwt, $datafilters = null)
    {
        ...
        if ($this->_dbSelected  == 'customer_jetsmart') {
            ...
            if(substr($survey, 3, 3) != 'vue'){
                ...
                $Generat = ['filter' => 'Generacion', 'datas' => $generation];

                $response = ['filters' => [(object)$Gender, (object)$Generat], 'status' => Response::HTTP_OK];
            ...
            if(substr($survey, 3, 3) == 'vue'){
                ...
                $data = DB::select("SELECT DISTINCT(hasbag)
                FROM $this->_dbSelected.".$db."_start");

                $hasbag = ['filter' => 'Entrega Equipaje', 'datas' => $this->contentfilter($data, 'hasbag')];

                $data = DB::select("SELECT DISTINCT(hasbag)
                FROM $this->_dbSelected.".$db."_start");

                $hasach = ['filter' => 'Check-In', 'datas' => $this->contentfilter($data, 'hasbag')];
                
                $data = DB::select("SELECT DISTINCT(hasbag)
                FROM $this->_dbSelected.".$db."_start");

                $haspbd = ['filter' => 'Embarque Prioritario', 'datas' => $this->contentfilter($data, 'hasbag')];

                $response = ['filters' => [(object)$hasbag, (object)$hasach, (object)$haspbd], 'status' => Response::HTTP_OK];
    ...
    private function cbiPreviousPeriod($table,$dateEnd, $dateIni, $indicador, $datafilters)
    {
        ...
        if ($this->_dbSelected == 'customer_jetsmart' || $this->_dbSelected == 'customerscoops_dentalia') {
    ...
    { 
        ...
        if (!empty($data)) {
            if(substr($table, 6, 3) != 'jet' && substr($table, 6, 3) != 'den'){       
                foreach ($data as $key => $value) {
                    if ($struct != 'one') {
                        $graphCSAT[] = [
    ...
    {
        if ($filter && substr($filter, 0, 4) != ' AND')
            $filter = " AND $filter";
        ...
        $data = DB::select("SELECT
                            ...
                            FROM $this->_dbSelected.$table as a
                            INNER JOIN $this->_dbSelected." . $table . "_start as b on a.token = b. token 
                            WHERE date_survey BETWEEN '$dateEnd' AND '$dateIni' $activeP2 $filter
                            GROUP BY a.mes
                            ORDER BY date_survey asc");
    ...
    { 
        ...
        $data = DB::select("SELECT (COUNT(if($indicador between   $this->_minMaxCes and $this->_maxMaxCes  , $indicador, NULL)) - 
                            ...
                            a.mes, a.annio, date_survey, $this->_fieldSex, SUBDATE(date_survey, WEEKDAY(date_survey)) as mondayWeek 
                            FROM $this->_dbSelected.$table as a
                            INNER JOIN $this->_dbSelected." . $table . "_start as b on a.token = b. token 
                            WHERE $where $activeP2 $datafilters
                            GROUP BY $group
    ...
    private function cbiResp($db, $datafilters, $dateIni, $dateEnd, $key = 'cbi', $name = 'CBI')
    {
        ...
        if(substr($db,6,3) == 'jet' || substr($db,6,3) == 'den'){
    ...
    private function imagen($client, $filterClient,$nameEncuesta, $table = null){
        ...
        if($client == 'den' &&  $filterClient != 'all'){
            return   "<div style='display:flex; flex-direction:column'><span><span style='color:rgb(23, 199, 132)'>Hola </span>¡Este es tu Dashboard de la Encuesta $nameEncuesta! <img width='120px' src='$this->_imageClient'/></span><span style='display:flex; justify-content:flex-start;align-items:center; gap:10px; margin-top:10px'></span></div>";
        }
      
        return  "<div style='display:flex; flex-direction:column'><span><span style='color:rgb(23, 199, 132)'>Hola </span>¡Este es tu Dashboard Consolidado de $nameEncuesta!</span><span style='display:flex; justify-content:flex-start;align-items:center; gap:10px; margin-top:10px'><img width='250px' src='$this->_imageBanVid'/></span></div>";
    }
    ...
    protected function ces($db, $dateIni, $dateEnd, $ces, $datafilters=null){
        ...
        if($str == 'ges' || $str == 'eri' || $str == 'com' || $str == 'cpe' || substr($db, 6, 7) =='den_rel'){
    ...
    private function OrdenAerolineas($db, $dateIni, $dateEnd){
        ...
        $values['Preferencia'][0][0]['Total']                = $data[0]->total;
        $values['Preferencia'][0][0]['rowSpan']              = ['cells' => 3, 'key' => "Total"];
        
        if($data[0]->total != 0)
            $values['Preferencia'][0][0]['pos1']  = round($data[0]->pos1 * 100 / $data[0]->total).'%';

        if($data[0]->total == 0)
            $values['Preferencia'][0][0]['pos1']  = 'N/A';
    ...
    private function BrandAwareness($db, $dateIni, $dateEnd){
        ...
        $values['Reconocimiento'][0][0]['Indicator']            = 'Resultado';
        $values['Reconocimiento'][0][0]['Total']                = $suma;
        $values['Reconocimiento'][0][0]['rowSpan']              = ['cells' => 3, 'key' => "Total"];
       
        if($suma != 0){
            foreach ($data[0] as $index => $dato) {              
                $values['Reconocimiento'][0][0][$index]             = ROUND(($dato*100)/$suma).'%';
            }
        }

        if($suma == 0){
            foreach ($aerolineas as $index) {              
                $values['Reconocimiento'][0][0][$index]             = 'N/A';
            }
        }
    ...
    {   
        if ($request->get($fieldurl) === null || $request->get($fieldurl) != 0){
            return '';
        }
        if ($request->get($fieldurl) || $request->get($fieldurl) == 0) {
            if($fieldurl != 'Generacion'){
                if ($where != '') {
            ...
            if($fieldurl == 'Generacion'){
                $generation = [
                    'Z'     => 'BETWEEN 14 AND 22',
                    'MILLE' => 'BETWEEN 23 AND 38',
                    'X'     => 'BETWEEN 39 AND 54',
                    'BB'    => 'BETWEEN 55 AND 73',
                    'SIL'   => 'BETWEEN 74 AND 91',
                 ];
                if ($where != '') {
                    $where = " AND TIMESTAMPDIFF(YEAR, $fieldbd, NOW()) " . $generation[$request->get($fieldurl)];
                }
                if ($where == '') {
                    $where = " TIMESTAMPDIFF(YEAR, $fieldbd, NOW()) " . $generation[$request->get($fieldurl)];
                }
            }
        }
        return $where;
    ...
    private function infofilters($request)
    {
        ...
        if(substr($request->survey,0,3) == 'jet'){
            ...
            $where .= $this->structfilter($request, 'hasbag',       'EntregaEquipaje',      $where);
            $where .= $this->structfilter($request, 'hasach',       'Check-In',             $where);
            $where .= $this->structfilter($request, 'haspbd',       'EmbarquePrioritario',  $where);
            $where .= $this->structfilter($request, 'birth_date',   'Generacion',           $where);
    ...
    protected function cardsPerformace($dataNps, $dataCsat, $survey, $datafilters, $dataCes = null, $dataCbi = null, $dataCcs = null)
    {
        $width = 6;
        $resp = [];
        ...
        if ($this->_dbSelected == 'customer_colmena' &&  substr($survey, 0, 3) == 'mut') {    
            ...
            $resp = [
                [
                    ...
                    "m2m"     => (int)isset($dataNps[sizeof($dataNps)-1]['values']['nps']) && isset($dataNps[sizeof($dataNps)-2]['values']['nps'])?$dataNps[sizeof($dataNps)-1]['values']['nps'] - $dataNps[sizeof($dataNps)-2]['values']['nps']:0,
                ],
                [
                    "name"    => $dataCsat['name'],
        ...
        if($this->_dbSelected == 'customerscoops_dentalia'){
            $width = 12;
            $resp = [
                [
                    "name"    => $dataNps['name'],
        ...
        if ($this->_dbSelected != 'customer_jetsmart' && substr($survey, 0, 3) != 'mut' && $this->_dbSelected != 'customerscoops_dentalia') { 
            $resp = [
                        [
                            "name"    => $dataNps['name'],
                            "value"   => $dataNps['value'],
    ...
    private function cardNpsBanmedica($nameIndicatorPrincipal, $dataNPSGraph, $indicador = 'NPS')
    {
        ...
        return [
            ...
            "props" => [
                "icon" => "arrow-right",
                "text" => $nameIndicatorPrincipal != 'JetSmart' && $nameIndicatorPrincipal != 'Dentalia' ? "NPS • " . $nameIndicatorPrincipal : $indicador,
                "chart" => [
                    "fields" => [
                        [
                            "type" => "stacked-bar",
    ...
    public function arrayPushToValues($array, $valuesReferences, $keyReferences, $icon = [], $quantity = [], $expect = null){
        if($keyReferences == 'GAP'){
            foreach($valuesReferences as $key => $value){
                array_push($array, ['name' => $value, 'exp' => $expect]);
            }
        }

        if($keyReferences != 'GAP'){
            for ($i=0; $i < count($valuesReferences); $i++) { 
                array_push($array, ["icon" => count($icon) > 1 ? $icon[$i] : $icon[0], 
                                    "percentage"=> count($valuesReferences) > 1 ? $valuesReferences[$i] : $valuesReferences[0], 
                                    "quantity" => count($quantity) > 1 ? $quantity[$i] : '']);
            }
        }
    ...
    public function detailsDash($request, $jwt)
    {
        ...
        if(substr($request->survey,0,3) == 'jet' || substr($request->survey,0,3) == 'den'){
        ...
        if ($this->_dbSelected  == 'customer_banmedica') {
            ...
            $performance        = $this->cardsPerformace($dataNps, $dataCsat, $request->survey, $datafilters);
        ...
        if ($this->_dbSelected  == 'customer_jetsmart') {
            ...
            if ($db == 'adata_jet_via') {
                ...
                $datasSBT = $this->detailStats($db, 'cbi', $npsInDb, $csatInDb, 'laboral' , $endDateFilterMonth,$startDateFilterMonth, $filterClient, $datafilters, $jetNamesLab);
            ...
            if ($db == 'adata_jet_vue' || $db == 'adata_jet_com' || $db == 'adata_jet_cpe') {
                ...
                $detailsTaps = $this->GraphCSATAtributos($db, trim($request->survey), 'csat4',  $endDateFilterMonth, $startDateFilterMonth,  'one', 'two', $datafilters);
                $detailsProc = $this->GraphCSATAtributos($db, trim($request->survey), 'csat5',  $endDateFilterMonth, $startDateFilterMonth,  'one', 'two', $datafilters);
                $bo14 = $this->GraphCSATAtributos($db, trim($request->survey), 'csat6',  $endDateFilterMonth, $startDateFilterMonth,  'one', 'two', $datafilters);
            }
            ...
            if ($db == 'adata_jet_vue') {
                ...
                $dataCCSCard = $this->cbiResp($db, $datafilters, $dateIni, $dateEndIndicatorPrincipal, 'ccs', 'CCS');
            }
            ...
            if ($db == 'adata_jet_com') {
                $bo15 = $this->rankingTransvip($db, $datafilters, $dateIni, $startDateFilterMonth, 'opc_1', "Ingreso", 2, 4);
            }
            ...
            $dataCbi            = $this->cbiResp($db, $datafilters, $dateIni, $dateEndIndicatorPrincipal);
            ...
            $welcome            = $this->welcome(substr($request->survey, 0, 3), $filterClient,$request->survey, $db);
            $performance        = $this->cardsPerformace($dataNps, $dataCsat, $request->survey, $datafilters, $dataCes, $dataCbi, $dataCCSCard);
            ...
            $detailGender       = $detGene; 
            $detailGeneration   = $datasSBT;
            $datasStatsByTaps   = $detailsTaps;
        ...
        if ($this->getValueParams('_dbSelected')  == 'customerscoops_dentalia'){
            $name = 'Dentalia';
            $dataCes            = $this->ces($db, $dateIni, $dateEndIndicatorPrincipal, 'ces', $datafilters);
            ...
            $graphCSATDrivers   = $this->GraphCSATDriversTransvip($db, trim($request->survey), $dateIni, $startDateFilterMonth, null, 'two', $datafilters);
            $dataCesGraph       = $this->graphCes($db, date('m'), date('Y'), 'ces', $dateIni, $dateEnd,  $filterClient, 'two' ,$datafilters);
            $dataCbiGraph       = $this->graphCbi($db, date('m'), date('Y'), 'cbi', $dateIni, $dateEnd, $datafilters, 'two');

            $welcome            = $this->welcome(substr($request->survey, 0, 3), $filterClient,$request->survey, $db);
            $performance        = $this->cardsPerformace($dataNps, $dataCsat, $request->survey, $datafilters, $dataCes, $dataCbi);
            $npsConsolidado     = $this->cardNpsConsolidado($this->_nameClient, $dataNPSGraph, $this->ButFilterWeeks);
            $npsBan             = $this->cardNpsBanmedica($this->_nameClient , $dataCsatGraph, 'CSAT');
            $npsVid             = $this->cardNpsBanmedica($this->_nameClient , $dataCesGraph, 'CES');
            $csatJourney        = $this->cardNpsBanmedica($this->_nameClient , $dataCbiGraph, 'CBI');
            $csatDrivers        = null; 
            $cx                 = $this->cxIntelligence($request);
            $wordCloud          = $this->CSATJourney($graphCSATDrivers);
            $closedLoop         = null; 
